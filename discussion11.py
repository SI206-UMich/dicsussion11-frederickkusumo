...
def create_patients_table(cur, conn):
    ...
    cur.execute("CREATE TABLE Patients (id INTEGER PRIMARY KEY, name TEXT, species_id INTEGER, \
        age INTEGER, cuteness INTEGER, aggresiveness INTEGER)")
    conn.commit()


# ADD FLUFFLE TO THE TABLE
def add_fluffle(cur, conn):
    cur.execute("INSERT INTO Patients (id, name, species_id, age, cuteness, aggresiveness) VALUES (?,?,?,?,?,?)",
...
def add_pets_from_json(filename, cur, conn):
    ...
    for info in json_data:
        pet_id = count
        cur.execute("SELECT id FROM Species WHERE title = ?", (info['species'],))
        species = int(cur.fetchone()[0])   
        cur.execute("INSERT INTO Patients (id, name, species_id, age, cuteness, aggresiveness) VALUES (?,?,?,?,?,?)",
            (pet_id, info['name'], species, int(info['age']), int(info['cuteness']), int(info['aggressiveness'])))
        count += 1
    conn.commit()
...
def non_aggressive_pets(aggressiveness, cur, conn):
    non = []
    cur.execute(f'SELECT name FROM Patients WHERE aggresiveness <= {aggressiveness}')
    rows = cur.fetchall()
    for row in rows:
        non.append(row)
    return non
